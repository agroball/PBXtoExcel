...
require_once("configRO.php");
include("sesvars_TMSORO.php");
...
// выставляем дату "жестко"
               ...
               $objWriter->save(dirname(__FILE__)."/dir2"."/".$name);
               $objPHPexcel = PHPExcel_IOFactory::load(dirname(__FILE__)."/dir2"."/".$name);   
...
//3244 ТМСО2 ряд 2
		if(strpos($row[2],"Ð¢ÐœÐ¡Ðž")!== FALSE) {
 			$answered22++;
...
//3945 ряд 5
                 if(strpos($row[2],"Ð¢ÐœÐ¡Ðž CPU") !== FALSE){
                        $answeredt22++;
...
$query.= "left join asterisk.incoming aa on q.info2=aa.cidnum where q.datetime>=$start and q.datetime<=$end and n.queue=9980 and (e.event='ENTERQUEUE') ";
...
$query1.= "and n.queue=9980 and (e.event='ENTERQUEUE') group by q.info2 order by count(*) desc limit 5";
...
$query2.= "and q.datetime<='$end' and n.queue=9980 and (e.event='COMPLETECALLER' or e.event='COMPLETEAGENT' or e.event='ABANDON')";
...
$query3.= "where q.datetime>='$start' and q.datetime<='$end' and n.queue=9980 and (e.event='ENTERQUEUE') and aa.description like '%TSIS%'";
...
$query3 = "select count(*) as 'counts' from ";
$query3.= "queue_stats q left join qname n on q.qname=n.qname_id left join qevent e on q.qevent=e.event_id left join qagent a on q.qagent=a.agent_id left join asterisk.incoming aa on q.info2=aa.cidnum ";
$query3.= "where q.datetime>='$start' and q.datetime<='$end' and n.queue=9980 and (e.event='ENTERQUEUE') and aa.description like '%GIBDD%'";
$res3 = consulta_db($query3,$DB_DEBUG,$DB_MUERE);
...
$query3 = "select count(*) as 'counts' from ";
$query3.= "queue_stats q left join qname n on q.qname=n.qname_id left join qevent e on q.qevent=e.event_id left join qagent a on q.qagent=a.agent_id left join asterisk.incoming aa on q.info2=aa.cidnum ";
$query3.= "where q.datetime>='$start' and q.datetime<='$end' and n.queue=9980 and (e.event='ENTERQUEUE') and aa.description like '%MTTS%'";
$res3 = consulta_db($query3,$DB_DEBUG,$DB_MUERE);
...
$query3 = "select count(*) as 'counts' from ";
$query3.= "queue_stats q left join qname n on q.qname=n.qname_id left join qevent e on q.qevent=e.event_id left join qagent a on q.qagent=a.agent_id left join asterisk.incoming aa on q.info2=aa.cidnum ";
$query3.= "where q.datetime>='$start' and q.datetime<='$end' and n.queue=9980 and (e.event='ENTERQUEUE') and aa.description like '%Evacuation%'";
$res3 = consulta_db($query3,$DB_DEBUG,$DB_MUERE);
...
$query3 = "select count(*) as 'counts' from ";
$query3.= "queue_stats q left join qname n on q.qname=n.qname_id left join qevent e on q.qevent=e.event_id left join qagent a on q.qagent=a.agent_id left join asterisk.incoming aa on q.info2=aa.cidnum ";
$query3.= "where q.datetime>='$start' and q.datetime<='$end' and n.queue=9980 and (e.event='ENTERQUEUE') and aa.description like '%RAS%'";
$res3 = consulta_db($query3,$DB_DEBUG,$DB_MUERE);
...
$query3 = "select count(*) as 'counts' from ";
$query3.= "queue_stats q left join qname n on q.qname=n.qname_id left join qevent e on q.qevent=e.event_id left join qagent a on q.qagent=a.agent_id left join asterisk.incoming aa on q.info2=aa.cidnum ";
$query3.= "where q.datetime>='$start' and q.datetime<='$end' and n.queue=9980 and (e.event='ENTERQUEUE') and q.info2 in ('74952499390','74952491625')";
$res3 = consulta_db($query3,$DB_DEBUG,$DB_MUERE);
...
//				$to = 'puy_t@unitoll.ru;dos-santos_m@unitoll.ru;nikiyan_o@unitoll.ru;erdeli_g@unitoll.ru;krotov_a@unitoll.ru,shvec_k@unitoll.ru;gradoboev_e@unitoll.ru;lemsadze_a@unitoll.ru;poberezhets_m@unitoll.ru;Andrey.Kapitanov@nwcc-msp.ru;borisenkova_n@unitoll.ru;dmitrikova_v@unitoll.ru;fomin_d@unitoll.ru;Aleksey.Chernyshov@msp-highway.ru;mamedov_s@unitoll.ru;taran_e@unitoll.ru';
				...
				$file = (dirname(__FILE__)."/dir2"."/".$name);
				$filewin = ("/mnt"."/smb"."/S5"."/Prihod"."/".$name);
				copy($file,$filewin);

		

				$namero = 'docxfromdb '.date("Y-m-d", time()-86400).'.xlsx';				
				$filero =(dirname(__FILE__)."/rojs"."/S5"."/".$namero);
				$filewinro = ("/mnt"."/smb"."/S5"."/Prihod"."/".$namero);
